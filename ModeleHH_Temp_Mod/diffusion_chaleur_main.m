...
Tw_list = [0.03 0.04 0.05 0.06 0.07];
v_list_tot = [];
...
for value_tw=1:length(Tw_list)
    ...
    while ts<=stop,
        for step=1:npoints+1,
                 TT1(step,ts)=Tm*exp(-(step-0.45/dx)^2/(2*Tw^2));   % fonction qui code pour la diffusion de la température dans le matériau
        end
    ...
    t=2;
	    ...
	    for step=2:npoints+1;
            v=V(step,t-1);
            v1=V(step-1,t-1);
            v3=V(step+1,t-1);   % il prend la valeur avant et après le step pour calculer 
            ve=Ve(step,t-1);
            ve1=Ve(step-1,t-1);
            ve3=Ve(step+1,t-1);
            m=M(step-1,t-1);
      ...
      for step=1:npoints;
          ...
          M(step,t)=M(step,t-1)+dt*(km1(step,t-1)+2*km2(step,t-1)+...            %  (1  2  2  1)/6
          ...
          if step == round(npoints/2);
              ...
              v_center = [v_center, V(npoints/2,t)+V_rest];
              %disp(v_list)
          end
...
ylabel("XXXX");
xlabel("XXXXX");
title("TT1 with different Tw values");
legend("ELEMENT 1: " + Tw_list(1), "ELEMENT 2: " + Tw_list(2), "ELEMENT 3: " + Tw_list(3), "ELEMENT 4: " + Tw_list(4), "ELEMENT 5: " + Tw_list(5));

% 
% ylabel("XXXX");
% xlabel("XXXXX");
% title("AP with different Tw values");
% legend("ELEMENT 1: " + Tw_list(1), "ELEMENT 2: " + Tw_list(2), "ELEMENT 3: " + Tw_list(3), "ELEMENT 4: " + Tw_list(4), "ELEMENT 5: " + Tw_list(5));
