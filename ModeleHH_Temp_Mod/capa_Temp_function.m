...
function CC = capa_Temp_function(Tm_init)
    ...
    ts=20;
    number_temp_peak = 1;
    add_time = round(stop/number_temp_peak);
    ...
    for peak=1:number_temp_peak
        ts = 20;
        value_temp = T;
        peak = peak - 1;
        %while ts<=(Tr+20),
        while ts<=stop/number_temp_peak,
            if value_temp < 31
                    for step=1:npoints+1,
                        TT(step, ts + peak*add_time)= T + TT1(step,ts+ peak*add_time)*(ts-20)/(Tr) ;  % en un point la variation temporelle de T est linéaire
                    end
                    ts=ts+1;
                    value_temp = value_temp + TT(npoints/2, ts + peak*add_time);
            end 
             if value_temp >= 31
                for step=1:npoints+1,
                     TT(step,ts+ peak*add_time)=T +TT1(step,ts+ peak*add_time)*exp(-(ts-(Tr+20))/(100/dt));
                end
                 ts=ts+1;
                 value_temp = value_temp + TT(npoints/2, ts + peak*add_time);
            end 
